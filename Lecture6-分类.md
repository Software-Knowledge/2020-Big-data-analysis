...
3. 初始化优化问题:我们选择最大间距作为这个优化问题的最优解，也就是使得$\frac{2}{||w||}$的值最大，并且满足一下两个约束条件
   1. $(wx+b) >= 1, \forall$ x in class 1
   2. $(wx+b) <= -1, \forall$ x in class 2
...
3. 之所以以这种方式定义边距，是因为理论上的便利以及依赖于边距值的泛化误差范围的存在。
...
### 7.5.4. 最大距离的求解
1. Prediction = $sign(w*x+b)$
2. Confidence = $(w*x + b)y$
