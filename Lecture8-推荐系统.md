...
#### 6.1.2.1. 并行混合设计：加权
1. 根据权重计算和:$res_{weighted}(u, i) = \sum\limits_{k=1}\limits^n\beta_k * rec_k(u, i)$
...
4. 动态调整权重
   1. 从均匀分布的权重开始
   2. 对于每个用户，调整权重以最大程序地减少预测误差
- 假设爱丽丝实际上购买/点击了项目1和4:确定最小化平均绝对误差（MAE）的权重:随着rec2的权重增加，MAE会提高
...
$rec_{mixed} = \cup\limits_{k = 1}\limits^n<rec_k(u, i), k>$
...
10. 教育性
    ...
    4. 最终，随着客户的了解越来越多，他们能够做出更明智的购买决定
 1. 前述产生解释的目的可以相互关联
 2. 说服力+`->`信任-
 3. 有效性+`->`信任+
...
## 8.8. $F_1$度量
- $F_1$指标尝试将Precision和Recall合并为一个值以进行比较:可用于获得更平衡的性能视图
