...
$r_{j} = \sum\limits\limits_{i->j} \frac{r_i}{d_i}$
...
3. 高斯法消去只适用于小规模的例子，我们需要其他方法来应对大规模的Web图

## 3.4. PageRank的矩阵等式(核心)
1. $r^{(0)}$需要给定四个网页的初始的PR值

### 3.4.1. 随机邻接矩阵M
1. 页面i有$d_i$个出链
2. 如果i->j，则$M_{ji} = \frac{i}{d_i}$，不然$M_{ji}=0$
   1. M是随机邻接矩阵的列，列所有元素和为1
...
### 3.6.2. 存在性和唯一性
1. 随机游走理论（又称马尔可夫过程）的主要结果是：对于满足某些条件的图，平稳分布是唯一的，并且无论在时间t = 0时的初始概率分布如何，最终都会达到平稳分布
...
1. 所有的出链形成了一个环状结构
2. 随机游走被困在环中
3. 这个不是一个问题，但是不是我们想要的结果:这个环吸收走了模型的所有的重要性
...
4. Goolge Matrix A:$A = \beta M + (1 - \beta)[\frac{1}{N}]_{N * N}$
